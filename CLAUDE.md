...
- **Editor (`#admin`):** a **full-screen hash route** (visiting `#admin` hides the site, shows only the writing desk). Markdown textarea + a tactile-key toolbar + a pinned cheatsheet + a live preview that renders via the same `mdToHtml`. **Publish commits to GitHub for real** (contents API, live-array parse, sha-conflict retry) once `CONFIG.TOKEN_CIPHER` is set; while it's `null`, Publish falls back to local/in-memory. Image upload commits to `images/` and auto-fills the URL.
