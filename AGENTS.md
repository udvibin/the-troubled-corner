...
- **`index.html`** — the entire site. Internally, top to bottom:
  ...
  - last `<script>` — **BEHAVIOUR**: `mdToHtml()` (a tiny no-library Markdown renderer), the day-book renderers (`renderStream/renderArchive/renderListening/renderFooter`), **share cards** (`drawCard/shareCard` — the "pass it on" line paints an essay onto a canvas for Instagram/Twitter and copies its deep link), **the sounds** (`CUE`/`cue()` — four Web-Audio-synthesized cues: paper/press/tick/chime, footer switch, idea credited to Cuelume), wake/door/unfold clicks, the `#admin` writing desk, publishing.
...
- **Aesthetic: antique letterpress.** Aged-paper ivory `#f3efe3` (with a CSS patina: foxing stains + edge vignette + SVG grain, painted on a `position:fixed` `body::before` — iOS ignores `background-attachment:fixed`), warm ink `#1f1b16`, oxblood `#7c1f1f`. Type is **IM Fell DW Pica** everywhere, 20px desktop / 18px phones; **headings are Fell roman** (the small-caps cut was retired for headings). **Drop caps: Royal Initialen + Yinit**, either selectable per paragraph, insertable anywhere/multiple. **Vertical rhythm:** block gaps come from the spacing scale in `:root` (`--s1…--s6`, multiples of .8rem) — pick a step, don't invent a margin; micro-gaps under ~.5rem stay hand-tuned. No neon/CRT/glow/scanlines.
