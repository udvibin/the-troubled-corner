...
- **`index.html`** — the entire site. Internally, top to bottom:
  ...
  - last `<script>` — **BEHAVIOUR**: `mdToHtml()` (a tiny no-library Markdown renderer), the day-book renderers (`renderStream/renderArchive/renderListening/renderFooter`), **share cards** (`drawCard/shareCard` — the "pass it on" line paints an essay onto a canvas for Instagram/Twitter and copies its deep link), wake/door/unfold clicks, the `#admin` writing desk, publishing.
